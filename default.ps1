...
task compile {
    ...
    exec { msbuild $base_dir\regard-cube-collector.sln /p:Configuration=$config /verbosity:minimal /tv:4.0 /p:VisualStudioVersion=$visualStudioVersion }
}

task test -depends compile {
    "Testing"
    
    exec { & $base_dir\packages\NUnit.Runners.2.6.3\tools\nunit-console.exe $base_dir\CubeWorkerRoleTests\bin\$config\CubeWorkerRoleTests.dll }
}
...
task package -depends test {
    ...
    exec { msbuild $base_dir\regard-cube-collector.ccproj /t:Publish /p:Configuration=$config /verbosity:minimal /tv:4.0 /p:VisualStudioVersion=$visualStudioVersion }
}